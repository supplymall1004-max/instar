...
**Instagram SNS 프로젝트를 위한 마이그레이션:**

1. Supabase Dashboard → **SQL Editor** 메뉴
2. **"New query"** 클릭
3. 다음 순서로 마이그레이션 파일 실행:
   - `supabase/migrations/20251105013534_add_user_fields.sql`
   - `supabase/migrations/20251105013535_create_posts_table.sql`
   - `supabase/migrations/20251105013536_create_likes_table.sql`
   - `supabase/migrations/20251105013537_create_comments_table.sql`
   - `supabase/migrations/20251105013538_create_follows_table.sql`
   - `supabase/migrations/20251105013539_create_posts_storage.sql`
4. 각 파일 내용을 복사하여 붙여넣기 후 **"Run"** 클릭
5. 성공 메시지 확인

> 📖 **상세 가이드**: [데이터베이스 설정 가이드](./docs/database-setup.md) 참고

**생성되는 테이블:**
- `users`: Clerk 사용자와 동기화되는 사용자 정보 테이블 (username, avatar_url, bio 필드 포함)
- `posts`: 게시물 정보 테이블
- `likes`: 게시물 좋아요 테이블
- `comments`: 게시물 댓글 테이블
- `follows`: 사용자 팔로우 관계 테이블

**생성되는 Storage 버킷:**
- `posts`: 게시물 이미지 저장용 버킷 (공개, 5MB 제한)
